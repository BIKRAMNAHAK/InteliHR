...
import { View, Text, TextInput, TouchableOpacity, StyleSheet, Animated, KeyboardAvoidingView, Platform, ScrollView, ActivityIndicator, TouchableWithoutFeedback, Keyboard } from 'react-native';
...
const LoginWithOtp = () => {
  ...
  const animate = (anim, toVal) => Animated.timing(anim, { toValue: toVal, duration: 200, useNativeDriver: false }).start();
  ...
  const handleGetOtp = () => {
    ...
    dispatch(requestOtpAsync(input))
      ...
      .catch((err) => {
        showAlert("Error", err.message || "Something went wrong", "danger");
      })
    ...
    dispatch(loginUserWithOtp({ ...input, otp }))
      .then((res) => {
        if (res.status == "200") {
          ...
          showAlert("Login Failed", res.message, "danger");
        }
      })
      .catch((err) => {
        showAlert("Error", err.message || "Something went wrong", "danger");
  ...
  React.useEffect(() => {
    let interval;
    if (showOtpInput && timer > 0) interval = setInterval(() => setTimer(t => t - 1), 1000);
    else if (timer === 0) {
      setCanResend(true);
    ...
    <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
      <View style={{ flex: 1 }}>
        ...
        <AwesomeAlert
          ...
          confirmText="OK"
          confirmButtonColor={
            alertConfig.type === "success" ? "#4CAF50" : alertConfig.type === "warning" ? "#FFA000" : "#E53935"
          }
          onConfirmPressed={() => setAlertVisible(false)}
          contentContainerStyle={{ width: '85%', paddingHorizontal: 20, borderRadius: 10 }}
        />

        <TouchableOpacity onPress={() => navigation.goBack()} style={styles.backBtnAbsolute}>
          <MaterialCommunityIcons name="chevron-left" size={30} color="#000" /><Text style={styles.btnText}>Back</Text>
        </TouchableOpacity>
        ...
        <ScrollView contentContainerStyle={{ flexGrow: 1 }} keyboardShouldPersistTaps="always" keyboardDismissMode="on-drag">
          <KeyboardAvoidingView style={styles.container} behavior={Platform.OS === "ios" ? "padding" : "height"}>
            <View style={styles.card}>
              ...
              <View style={styles.inputWrapper}>
                ...
                <TextInput
                  ...
                  onChangeText={t => {
                    ...
                    if (numeric.length > 0) animate(animEmail, 1); else animate(animEmail, 0);
                  }}
              ...
              {!showOtpInput ? (
                <TouchableOpacity style={[styles.btn, (input.phone.length !== 10 || loading) && { opacity: 0.6 }]} onPress={handleGetOtp} disabled={loading || input.phone.length !== 10}>
...
const styles = StyleSheet.create({
  fullscreenOverlay: { ...StyleSheet.absoluteFillObject, backgroundColor: 'rgba(0,0,0,0.3)', justifyContent: 'center', alignItems: 'center', zIndex: 10 },
  container: { flex: 1, justifyContent: 'center', alignItems: 'center', paddingTop: 60, paddingVertical: 30 },
  ...
  inputWrapper: { marginVertical: 15, position: 'relative', justifyContent: 'center' },
  input: { borderWidth: 1, borderColor: '#ccc', borderRadius: 10, paddingLeft: 15, paddingRight: 45, paddingVertical: 12, fontSize: 16, color: '#000', backgroundColor: '#fff' },
  iconRight: { position: 'absolute', right: 10, top: 12, zIndex: 2 },
  iconLeft: { position: 'absolute', left: 10, top: 12, zIndex: 2 },
  btn: { backgroundColor: '#e53935', paddingVertical: 14, borderRadius: 10, marginTop: 10, elevation: 5, justifyContent: 'center', flexDirection: 'row' },
  btnText: { color: '#000', fontSize: 16, fontWeight: 'bold' },
  btnTextInput : { color: '#fff', fontSize: 16, fontWeight: 'bold'},
